...
void UEnemyQueryContext::ProvideContext(FEnvQueryInstance& QueryInstance, FEnvQueryContextData& ContextData) const
{
    ...
    AEnemyAIController* EnemyController = Cast<AEnemyAIController>((Cast<AActor>((QueryInstance.Owner).Get())->GetInstigatorController()));
    ...
    if(EnemyController)
    {
        ...
        if(TargetEnemy)
        {
            UEnvQueryItemType_Actor::SetContextHelper(ContextData, TargetEnemy);
        }
    }
}